...
def cadastro_view(request):
    ...
    return render(request, 'comdirect/cadastro.html', context)
...
def menu_view(request):
    ...
    # Indicador de Nível
    nivel_ativo = NivelAlugado.objects.filter(usuario=usuario, is_active=True).first()
    if nivel_ativo:
        nivel_cliente = nivel_ativo.nivel.nome_nivel
    else:
        nivel_cliente = "Básico"
        
    # Indicador de Depósito Total Aprovado
    ...
    context = {
        'niveis': niveis,
        'config': config,
        'nivel_cliente': nivel_cliente, # Adicionado para o indicador do menu
        'depositos_aprovados': depositos_aprovados_sum, # Adicionado para o indicador do menu
    }
...
def deposito_view(request):
    ...
    valor_deposito = None
    # Recupera o valor de comprovativo_enviado da sessão se existir (após o redirect)
    comprovativo_enviado = request.session.pop('comprovativo_enviado', False) 
    
    if request.method == 'POST':
        if 'proof' in request.FILES:
            # Lógica para processar o comprovativo (Etapa 2 -> Etapa 3)
            try:
                valor_deposito_str = request.POST.get('valor_deposito')
                banco_nome = request.POST.get('banco_selecionado_nome')
                # O 'depositor_name' não é usado na criação do modelo Deposito, 
                # mas mantido no request.POST se necessário para validação.
                ...
                # Define a variável na sessão e redireciona para limpar o POST
                request.session['comprovativo_enviado'] = True 
            ...
            # Lógica para a primeira etapa (Seleção de valor e banco)
            valor_deposito = request.POST.get('amount')
            banco_nome = request.POST.get('method')
            
            if not valor_deposito or not banco_nome:
                messages.error(request, 'Por favor, selecione um valor e um método de pagamento.')
                return redirect('deposito')
            
            try:
                # Tenta converter para Decimal para validação simples
                Decimal(valor_deposito)
            except:
                messages.error(request, 'Valor de depósito inválido.')
                return redirect('deposito')
    ...
    context = {
        ...
        'comprovativo_enviado': comprovativo_enviado, # Variável de sucesso da Etapa 3 (vindo da sessão)
    }
...
def saque_view(request):
    ...
    config = Config.objects.first()
    if not config:
        messages.error(request, "Configurações da comdirect não encontradas. Por favor, contate o suporte.")
        return redirect('menu')
...
def perfil_view(request):
    ...
    context = {
        ...
        # Adicionar saldos ao contexto para a renderização inicial
